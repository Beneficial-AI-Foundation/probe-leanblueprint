...
| **Verso v4.30 / v4.31 / v4.33** | Reads the `graphs[].nodes[]` schema, recognized via `vbpInternalSchemaVersion` **2** (v4.30), **3** (v4.31) and **8** (v4.33, which renamed the code-binding fields to `codeData.externalDecls` / `codeData.literateDeclarations`). Another generation still parses if it carries a graph, but warns that the version marker is unrecognized/absent. Pre-graph renderers (≤ v4.28) emit only a flattened preview manifest with no graph to score. |
